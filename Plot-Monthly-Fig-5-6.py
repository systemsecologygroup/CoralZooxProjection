...
# Sensitivity with respect to speed of acclimation Fig 5 and 6                                                                                     
count = 1
...
file_list = ["Results-final/Monthly/"] #### Specific folder for the sensitivity simulations "Sensitivity-RCP-monthly-2.py"
        ...
        for ind in xrange(len(Params)):
            ...
            for file_index in xrange(len(file_list)):
                ...
                if file_index == 0 and z == 0:
                    ...
                    N_List = (scale)*rawNum
                    plt.plot(N_List_orig[reg_N_index[z]]*ones(2), linspace(0, 1, 2), color = Color_list[0], linewidth = 4)
            ...
            if count not in (7, 8, 9):
                ...
                plt.xlabel(r'Speed of acclimation $N$ $\times$ $10^{13}$', fontsize = fsize2)
            ...
            if count in (1, 4, 7):
                ...
                sub.text(min(N_List) - 0.1e-12, 0.5, Fig_lab[count-1], fontsize = fsize)
                ...
                plt.text(min(N_List) - 0.035e-12, 0.5, Fig_lab[count-1], fontsize = fsize)
...
if fileLab == "Plus":
    plt.savefig("SensN-monthly-Plus.pdf", bbox_inches = 'tight')
else:
    plt.savefig("SensN-monthly-Minus.pdf", bbox_inches = 'tight')                                   
